...
Given a Lie algebra $\lie{g}$, the construction of its universal enveloping algebra 
$\algebra{U}(\lie{g})$ stays the same, no matter if $\lie{g}$ is finite-dimensional 
or not and always yields an infinite-dimensional, associative algebra if $\lie{g} 
\neq \{0\}$. If one wants to study $\algebra{U}(\lie{g})$ in a topological setting, 
one can use different ways to see it. The most geometric one is as invariant 
differential operators on the corresponding Lie group $G$. Another interpretation are 
compactly supported distributions near the unit element on the Lie group. Both 
approaches hold up to some extend also in infinite dimensions: While the first is  
mostly used in the theory of quantum groups \cite{pflaum.schottenloher:1998a}, the 
second allows some more generalizations to infinite dimensional $\lie{g}$, see e.g. 
...
Poincar\'e-Birkhoff-Witt Theorem: one can see it as the symmetric tensor algebra 
$\Sym^{\bullet}(\lie{g})$ with the non-commutative product from $\algebra{U}
(\lie{g})$. This approach was first discovered by Rasevski\u{i} 
\cite{rasevskii:1966a} and Goodman \cite{goodman:1971a}, but seems to have gone 
rather unnoticed. These first attempts were bound to finite-dimensional Lie algebras, 
while a younger work \cite{esposito.stapor.waldmann:2015a:pre}, from the point of 
view of deformation quantization, used a similar idea to construct an explicit 
topology in a slightly different way on enveloping algebras of certain classes of 
locally convex Lie algebras.

A very important aim of this last work was to construct a very coarse topology on 
$\algebra{U}(\lie{g})$, in order to get a large completion. Since the elements in 
$\algebra{U}(\lie{g})$ can be interpreted as polynomials, the elements in the 
completion are certain entire functions. One important result was, that nilpotent 
locally convex Lie algebras allow bigger completions: here, one can construct a 
topology, which contains exponentials series $\exp(\xi)$ for every $\xi \in \lie{g}$, 
while for non-nilpotent Lie algebras, this is not possible, at least with this 
particular construction.

The aim of this paper is to investigate further, when such a bigger completion and 
hence a coarser topology is possible. Therefore, we first restrict to the easier case 
of Banach-Lie algebras. We show that for general (Banach-) Lie algebras, the topology 
from \cite{esposito.stapor.waldmann:2015a:pre} is as coarse, as possible. Since 
moreover, in Banach-Lie algebras, there exists weaker forms of nilpotency, it is a 
neat question to ask whether topologies with bigger completions containing 
exponentials (which we want to call grouplike topologies on $\algebra{U}()\lie{g}$), 
can exist for quasi-nilpotent Banach-Lie algebras. It is the main result of this work 
to show that there is a particular form of quasi-nilpotency, which is equivalent to 
the existence of such a topology on $\algebra{U}(\lie{g})$.

Nilpotency is a very well-studied and well known phenomenon. Weaker forms of it in 
infinite-dimensional algebras, are however less studied. A big part of the work in 
this area deals with normed associative algebras, often for certain algebras of 
compact operators \cite{shulman.turovskii:2000a}, but it seems like also in this 
case, there is not yet a closed theory. Yet, different notions of quasi-nilpotency 
were worked out and links between them are found, see e.g. \cite{dixon:1991a, 
mueller:1994a, mueller:1997a}. Especially joint quasi-nilpotency of bounded subsets, 
so called topological nilpotency, is an interesting field of study. This idea 
originated from a work of Rota and Strang \cite{rota.strang:1960a}, and was carried 
on in \cite{dixon.mueller:1992a}. A particularly nice paper from Galindo and Palacios 
\cite{galindo.palacios:2012a} also deals with non-associative normed algebras. But 
still, besides Wojty\'nski's theorem that a Banach-Lie algebra is quasi-nilpotent if 
and only if it has a globally convergent Baker-Campbell-Hausdorff series 
\cite{wojtynski:1998a}, further particular results or even a good notion of different 
types of quasi-nilpotency for general Banach-Lie algebras do not seem to exist.

This work is organized as follows: In Section 2, we present the main techniques and 
the construction of the already mentioned topology on a universal enveloping algebra 
via the Poincar\'e-Birkhoff-Witt Theorem, as in 
\cite{esposito.stapor.waldmann:2015a:pre} and use this possibility to present a new 
result for locally convex, nilpotent Lie algebras in this context. Then we specify 
what kind of topology we would like to achieve on $\algebra{U}(\lie{g})$ and define 
projective grouplike topologies. Section 3 deals with different types of 
quasi-nilpotent Banach-Lie algebras and tries to transfer some of the notions from 
the associative case to the Lie case. We moreover show that an important result for 
associative Banach algebras is also valid for Banach-Lie algebras and finally give an 
example for each type of quasi-nilpotency. Section 4 contains the main result of this 
...
$\algebra{U}(\lie{g})$ in a constructive sense. Finally, in section 5, we state some 
immediate corollaries from our results and show some ways and limits for a 
generalization to locally convex Lie algebras.

Acknowledgements: I want to say thank you to...
...
(vi.)$ \cite{dixon.mueller:1992a}. This is also true for Banach-Lie algebras.
